...
t_exact = linspace(0,0.3);
time = [0 0.3];

%% Gleichungssystem mit expliziter Lösungsformel
...
for i=1:i_max
    y_dach(:,i+1) = (eye(2) - h(run)*C) * y_dach(:,i);
end

%% Plot der Funktionen (zwischen 1 und 2 waehlen)
if run == 1, figure('Name','Gleichungssystem mit expliziter Lösungsformel, y2','NumberTitle','off'); end
subplot(2,3,run);
hold on;
